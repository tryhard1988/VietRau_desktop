...
public class BangKeThuMuaHangHoa_Printer2
{
    private DataTable mData_dt, mSeller_dt;
    private List<KeyValuePair<int, DataTable>> mDataGroup;
    private int rowIndex = 0, groupIndex = 0; // để phân trang
    private int lineHeight = 23;
    private int mCountSTT;
    private int total_thanhtien = 0;
    private decimal total_soLuong = 0;
    private DateTime mDate;
    private string mCode;
    public BangKeThuMuaHangHoa_Printer2(string code, DataTable data_dt)
    {
        DataView dv = new DataView(data_dt);
        dv.RowFilter = $@"FarmSourceCode = '{code}'";

        this.mData_dt = dv.ToTable();

        if (mData_dt.Rows.Count > 0)
        {
            mDate = Convert.ToDateTime(mData_dt.Rows[0]["TransactionDate"]);
        }

        mDataGroup = mData_dt.AsEnumerable().Where(r => r["SellerID"] != DBNull.Value).GroupBy(r => r.Field<int>("SellerID"))
                                        .Select(g =>
                                        {
                                            ...
                                            return new KeyValuePair<int, DataTable>(g.Key, dt);
                                        }).ToList();
        this.mCountSTT = 1;
        total_thanhtien = 0;
        mCode = code;
    ...
    {
        mSeller_dt = await SQLStore_Kho.Instance.GetSupplierAsync();
    }
