...
{
    public partial class EditExcelFormSalary : Form
    {
        public EditExcelFormSalary()
        {
            InitializeComponent();
        ...
        {
            ImportDataGridViewToSql(dataGV, SQLManager_QLNS.Instance.ql_NhanSu_conStr(), "Employee");
        }
        ...
        private void ImportDataGridViewToSql(DataGridView dgv, string connectionString, string tableName)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                foreach (DataGridViewRow row in dataGV.Rows)
                {
                    ...
                    int baseSalary = Convert.ToInt32(row.Cells["BaseSalary"].Value);

                    string sql = @"
            INSERT INTO EmployeeSalaryInfo
            (EmployeeCode, Month, Year, BaseSalary, InsuranceBaseSalary, Note)
            VALUES
            (@EmployeeCode, @Month, @Year, @BaseSalary, @InsuranceBaseSalary, @Note)";
                    ...
                    using (SqlCommand cmd = new SqlCommand(sql, conn))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@InsuranceBaseSalary", baseSalary);
                        cmd.Parameters.AddWithValue("@Note", "Data từ excel");
                        cmd.ExecuteNonQuery();
                    }
                }
