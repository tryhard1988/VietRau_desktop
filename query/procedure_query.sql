    ...
    BEGIN TRY
        ...
        WHEN MATCHED THEN
            UPDATE SET 
                ...
                target.IsInsuranceRefund = src.IsInsuranceRefund,
        ...
        WHEN NOT MATCHED THEN
            INSERT (
                ...
                OvertimeSalary, LeaveSalary, DeductionAmount, IsInsuranceRefund, CreatedAt
            )
            VALUES (
                ...
                src.OvertimeSalary, src.LeaveSalary, src.DeductionAmount, IsInsuranceRefund, GETDATE()
            );
    END TRY
    BEGIN CATCH
        DECLARE @ErrMsg NVARCHAR(4000) = ERROR_MESSAGE();
...
CREATE PROCEDURE sp_GetSalarySummaryByYear
    @Year INT
AS
BEGIN
    SET NOCOUNT ON;
    ...
    SELECT 
        ...
        SUM(NetInsuranceSalary)      AS TotalNetInsuranceSalary,
        SUM(InsuranceAllowance)      AS TotalInsuranceAllowance,
        SUM(NonInsuranceAllowance)   AS TotalNonInsuranceAllowance,
        SUM(OvertimeSalary)          AS TotalOvertimeSalary,
        SUM(LeaveSalary)             AS TotalLeaveSalary,
        SUM(DeductionAmount)         AS TotalDeductionAmount,
...
CREATE PROCEDURE dbo.UpdateOrdersBulk
    @OrderUpdates dbo.OrderUpdateType READONLY
AS
BEGIN
    SET NOCOUNT ON;
    ...
    SET 
        o.PCSReal = u.PCSReal,
        o.NWReal = u.NWReal,
        o.CartonNo = u.CartonNo,
        o.CartonSize = u.CartonSize,
        o.CustomerCarton = u.CustomerCarton
    FROM Orders o
    JOIN @OrderUpdates u ON o.OrderId = u.OrderId;
END

CREATE TYPE dbo.OrderLotUpdateType AS TABLE
(
    ExportCodeID INT,
    SKU INT,
    LOTCode NVARCHAR(50) NULL,
    LOTCodeComplete NVARCHAR(50) NULL
);

CREATE PROCEDURE dbo.UpsertOrdersLotCodesBySKU
    @Updates dbo.OrderLotUpdateType READONLY
AS
BEGIN
    SET NOCOUNT ON;
    ...
    SET 
        o.LOTCode = u.LOTCode,
        o.LOTCodeComplete = u.LOTCodeComplete
    FROM Orders o
    INNER JOIN ProductPacking p ON o.ProductPackingID = p.ProductPackingID
    INNER JOIN @Updates u ON o.ExportCodeID = u.ExportCodeID
                         AND p.SKU = u.SKU;
END

CREATE TYPE dbo.OrderTotalUpdateType AS TABLE
(
    ExportCodeID INT,
    ProductPackingID INT,
    NetWeightFinal DECIMAL(18,2) NULL
);

CREATE PROCEDURE dbo.UpsertOrdersTotalList
    @Updates dbo.OrderTotalUpdateType READONLY
AS
BEGIN
    SET NOCOUNT ON;

    MERGE INTO OrdersTotal AS target
    USING @Updates AS source
    ON target.ExportCodeID = source.ExportCodeID
       AND target.ProductPackingID = source.ProductPackingID
    WHEN MATCHED THEN
        UPDATE SET 
            target.NetWeightFinal = source.NetWeightFinal
    WHEN NOT MATCHED BY TARGET THEN
        INSERT (ExportCodeID, ProductPackingID, NetWeightFinal)
        VALUES (source.ExportCodeID, source.ProductPackingID, source.NetWeightFinal);
END
